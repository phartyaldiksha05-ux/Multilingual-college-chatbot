...
def detect_course_type(text: str):
    t = text.lower()
    if any(k in t for k in ["btech", "[b.tech](http://b.tech)", "undergraduate", "ug", "12th"]):
        return "btech"
    if any(k in t for k in ["mca"]):
        return "mca"
    if any(k in t for k in ["mtech", "[m.tech](http://m.tech)", "pg"]):
        return "mtech"
    return "general"
...
"""
    ...
    [vectorstore.save](http://vectorstore.save)_local(index_path)
    print("\n" + "=" * 50)
